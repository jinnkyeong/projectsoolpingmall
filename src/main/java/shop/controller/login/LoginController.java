...
public class LoginController {
	...
	@Autowired
	public LoginController(LoginDao loginDao, HashingPasswd hashingPasswd) {
	...
	public String postLogin(@ModelAttribute UserBean userBean, HttpServletRequest request, Model model)
		...
		if (dto != null) {
			...
			System.out.print(userBean.getReferer());
			String rf = userBean.getReferer();
			if(rf.contains("signup")||rf.contains("findId")||rf.contains("findPasswd")) {
				return "redirect:/"; // 회원가입,아이디,비밀번호 찾기는 로그인 성공후 메인페이지로
			} else {
				return "redirect:"+ userBean.getReferer(); // 그 외에는 로그인 성공후 이전페이지로 이동		
			}
